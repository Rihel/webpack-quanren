    ...
    var methods = {
        pageInit: function(options) {
            ...
            return $(this).each(function(k, v) {
                ...
                methods.pageData(_v, opts.current, opts);
            })
        },
        pageData: function(_v, _current, opts) {
            /**
             * [opts this plug propertys]
             * @type {Obeject}
             */
            ...
            if (opts.ajaxSetData && (typeof(opts.ajaxSetData) === 'boolean')) {
                if (opts.url !== '' && typeof(opts.url) === 'string') {
                    ajaxOpts = methods.ajaxData(opts.url, _current, opts);
        ...
        ajaxData: function(_url, _current, opts) {
            /**
            ...
            var userparams = $.fn.zPager.defaults.userData;

            console.log('用户请求的参数', opts);
            return (function() {
                var parms = { 'total': _total, 'rows': [] };
                $.ajax({
                    url: _url,
                    type: 'get',
                    data: $.extend({}, { "pageNo": _current }, opts.userData),
                    dataType: 'json',
                    cache: false,
                    async: false,
                    xhrFields: {
